...
DATA_PATH=${DATA_PATH:-"/path/to/longv_desc.json"}
...
OUTPUT_DIR=${OUTPUT_DIR:-"/path/to/output/${run_name:-stage2_salova}/"}

deepspeed --master_addr 127.0.0.1 --master_port 12336 --include localhost:0,1,2,3,4,5,6,7 llava/train/train_mem.py \
    ...
    --mm_tunable_parts mm_mlp_adapter,mm_vision_resampler,mm_mlp_selector,mm_vision_tower,mm_text_tower,mm_language_model \
    --pretrain_mm_mlp_adapter $pretrain_path \
