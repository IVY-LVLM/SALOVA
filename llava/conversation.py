...
class Conversation:
    ...
    def get_prompt(self):
        ...
        elif self.sep_style == SeparatorStyle.LLAMA_3_1_infer:
            ret = "" if self.system == "" else "<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n" + self.system + self.sep
            for role, message in messages:
                if message:
                    if type(message) is tuple:
                        message, images, _ = message
...
conv_qwen_2_5_infer = Conversation(
    system="""<|im_start|>system"
You are a helpful assistant.""",
    roles=("<|im_start|>user", "<|im_start|>assistant"),
    version="qwen_2_5_infer",
    messages=[],
    offset=0,
    sep_style=SeparatorStyle.CHATML,
    sep="<|im_end|>",
...
conv_llava_llama_3_2 = Conversation(
    system="You are a helpful language and vision assistant. " "You are able to understand the visual content that the user provides, and assist the user with a variety of tasks using natural language.",
    #roles=("<|start_header_id|>user", "<|start_header_id|>assistant"),
    roles=("user", "assistant"),
    version="llama_3_2",
    ...
    tokenizer=safe_load_tokenizer("meta-llama/Llama-3.2-3B-Instruct"),
    stop_token_ids=[128009, 128008, 128001],
)

conv_llava_llama_3_2_infer = Conversation(
    system="You are a helpful language and vision assistant. " "You are able to understand the visual content that the user provides, and assist the user with a variety of tasks using natural language.",
    roles=("<|start_header_id|>user<|end_header_id|>\n", "<|start_header_id|>assistant<|end_header_id|>\n"),
    version="llama_3_2_infer",
    messages=[],
    offset=0,
    sep="<|eot_id|>",
    sep_style=SeparatorStyle.LLAMA_3_1_infer,
    stop_token_ids=[128009, 128008, 128001],
)
...
conv_templates = {
    ...
    "qwen_2_5_infer": conv_qwen_2_5_infer,
    "gemma_instruct": conv_gemma_instruct,
    "phi_3": conv_phi_3,
    "phi_3_5": conv_phi_3,
    # "llama_3": conv_llava_llama_3,
    # "llama_3_1": conv_llava_llama_3_1,
    "llama_3_2": conv_llava_llama_3_2,
    "llama_3_2_infer": conv_llava_llama_3_2_infer,
}
