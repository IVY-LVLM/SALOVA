...
def load_pretrained_model(model_path, model_base=None, model_name=None, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if "llava" in model_name.lower() or "qwen" in model_name.lower() or "phi" in model_name.lower() or "llama" in model_name.lower() or is_multimodal:
        # Load LLaVA model
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "phi" in model_name.lower():
                from llava.model.language_model.llava_phi import LlavaPhiConfig

                lora_cfg_pretrained = LlavaPhiConfig.from_pretrained(model_path)
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                model = LlavaPhiForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            ...
            if "phi" in model_name.lower():
                ...
                model = LlavaPhiForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            elif "qwen" in model_name.lower():
                tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
                cfg_pretrained = AutoConfig.from_pretrained(model_path)
            ...
            if "phi" in model_name.lower():
                ...
                model = LlavaPhiForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, attn_implementation=attn_implementation, **kwargs)    
                ...
                model = LlavaLlamaForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=False, attn_implementation=attn_implementation, **kwargs)
