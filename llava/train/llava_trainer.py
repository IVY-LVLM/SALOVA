...
class LLaVATrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        outputs, sim_loss, total_acc, avg_acc = model(**inputs)
        ...
        if outputs is not None:
            ...
            # print(f"[*] loss: {loss.item()} / sim_loss: {sim_loss.item()} / total_acc: {total_acc.item()}" )

            if self.args.local_rank == 0:
                if sim_loss:
                    wandb.log({"ar_loss": loss.item(), "sim_loss": sim_loss.item(), "retrieval_acc": total_acc.item()}, step=self.state.global_step)
                else:
                    wandb.log({"ar_loss": loss.item(),}, step=self.state.global_step)
                if avg_acc:
                    wandb.log({"avg_retrieval_acc": avg_acc.item()}, step=self.state.global_step)

            if model.eval_pointer % 100 == 0:
                l_text = self.tokenizer.decode(inputs["labels"][0][~inputs["labels"][0].eq(-100)])
                o_text = self.tokenizer.decode(torch.argmax(outputs["logits"][0][256:], dim=1))
                rank0_print(f"[*] GT: {l_text}")
                rank0_print(f"[*] PRED: {o_text}")

        if sim_loss:
            loss = loss.to(sim_loss.device)
            loss += sim_loss
    ...
    def create_accelerator_and_postprocess(self):
        ...
        accelerator_kwargs = InitProcessGroupKwargs(timeout=timedelta(seconds=7200))
        rank0_print("Setting NCCL timeout to 2hrs to avoid running errors.")
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        elif self.args.group_by_source:
            return self.train_dataset.get_source_grouped_indices(
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_text_tower_lr is not None:
                lr_mapper["text_tower"] = self.args.mm_text_tower_lr
                rank0_print("text tower lr set : {}".format(self.args.mm_text_tower_lr))
            if self.args.mm_vision_tower_lr is not None:
                lr_mapper["vision_tower"] = self.args.mm_vision_tower_lr
                rank0_print("vision tower lr set : {}".format(self.args.mm_vision_tower_lr))
            if len(lr_mapper) > 0:
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        # if getattr(self.args, "tune_mm_mlp_adapter", False) or (hasattr(self.args, "mm_tunable_parts") and (len(self.args.mm_tunable_parts.split(",")) == 4 
        #     and ("mm_mlp_adapter" in self.args.mm_tunable_parts or "mm_vision_resampler" in self.args.mm_tunable_parts or "mm_mlp_selector" in self.args.mm_tunable_parts  or "mm_text_tower" in self.args.mm_tunable_parts))
        # ):
        if True:
